...
async function setup() {
  ...
  const token = await mint(SECRET, { id: 'tok1', agent: 'BenefitBot', fields: ['identity.national_id', 'health.diagnosis_code'], purpose: 'benefit_claim', expiresAt: Date.now() + 60_000 });
  auth.register(token);
...
test('out-of-scope read is denied', async () => {
  const { broker, token } = await setup();
  const r = await broker.read(token, 'health.full_history', 'benefit_claim', 'HelperBot');
...
test('purpose mismatch is denied', async () => {
  const { broker, token } = await setup();
  const r = await broker.read(token, 'health.diagnosis_code', 'marketing', 'BenefitBot');
  assert.equal(r.ok, false);
...
test('read after revoke is denied — provably', async () => {
  ...
  assert.equal((await broker.read(token, 'health.diagnosis_code', 'benefit_claim', 'BenefitBot')).ok, true);
  auth.revoke(token.id);
  const after = await broker.read(token, 'health.diagnosis_code', 'benefit_claim', 'BenefitBot');
...
test('expired token is denied', async () => {
  ...
  const passport = createPassport();
  const broker = createBroker({ passport, auth });
  const token = await mint(SECRET, { id: 'old', agent: 'BenefitBot', fields: ['health.diagnosis_code'], purpose: 'benefit_claim', expiresAt: 1000 });
  auth.register(token);
  const r = await broker.read(token, 'health.diagnosis_code', 'benefit_claim', 'BenefitBot');
