...
test('chain verifies for honest entries', async () => {
  ...
  await l.append({ actor: 'BenefitBot', action: 'read', field: 'health.diagnosis_code', purpose: 'benefit_claim', decision: 'ALLOW', value: 'ICD-10 G35' });
  await l.append({ actor: 'HelperBot', action: 'read', field: 'health.full_history', purpose: 'benefit_claim', decision: 'DENY', reason: 'out-of-scope' });
...
test('tampering with a past entry is detected', async () => {
  const l = createLedger();
  await l.append({ actor: 'BenefitBot', action: 'read', field: 'health.diagnosis_code', purpose: 'benefit_claim', decision: 'ALLOW', value: 'ICD-10 G35' });
  await l.append({ actor: 'BenefitBot', action: 'read', field: 'income.monthly', purpose: 'benefit_claim', decision: 'ALLOW', value: 'CLP 520.000' });
...
test('export produces a consent-receipt-style object', async () => {
  ...
  assert.equal(doc.entries[0].field, 'health.diagnosis_code');
});
