...
const base = { id: 'tok1', agent: 'BenefitBot', fields: ['identity.national_id', 'income.monthly', 'health.diagnosis_code'], purpose: 'benefit_claim', expiresAt: 10_000 };

test('a freshly minted token verifies', async () => {
  const t = await mint(SECRET, base);
  const v = await verify(SECRET, t);
  assert.equal(v.ok, true);
  assert.deepEqual(v.fields.sort(), ['health.diagnosis_code', 'identity.national_id', 'income.monthly']);
  assert.equal(v.expiresAt, 10_000);
});

test('tampering with root fields breaks verification', async () => {
  const t = await mint(SECRET, base);
  t.fields = [...t.fields, 'payment.history'];
  const v = await verify(SECRET, t);
...
test('attenuation narrows fields and cannot widen', async () => {
  const t = await mint(SECRET, base);
  const narrowed = await attenuate(t, { fields: ['health.diagnosis_code'] });
  const v = await verify(SECRET, narrowed);
  assert.equal(v.ok, true);
  assert.deepEqual(v.fields, ['health.diagnosis_code']);

  const widened = await attenuate(t, { fields: ['identity.national_id', 'income.monthly', 'health.diagnosis_code', 'payment.history'] });
  const vw = await verify(SECRET, widened);
  assert.equal(vw.ok, true);
  assert.equal(vw.fields.includes('payment.history'), false);
});
